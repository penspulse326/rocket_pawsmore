...
import { foodCategory } from '@/common/constants/formText';
...
const FoodInputs: React.FC<FoodInputsType> = ({ list, onChange: handleFoodChange }) => {
      ...
      {list.map((food: FoodType, index) => (
        <li key={`food-${index}`} className='flex items-center gap-1'>
          ...
          <input
            ...
            className='mr-1 w-16 rounded-[10px] border border-stroke px-2 py-1 outline-note'
          />
          {/* 按鈕 */}
